...
try:
    npzfile = np.load('pymerlin_coefs.npz')
...
if is_python3:
    regressors_path = "pymerlin_regressors_py3.pkl"
else:
    regressors_path = "pymerlin_regressors_py2.pkl"
...
try:
    npzfile = np.load('pymerlin_scaler_params.npz')
    scale   = npzfile['scale']
    mean    = npzfile['mean']
except:
...
for i in range(names.size):
    coefs = (Us*harmonic_coef_k.T).T.sum(axis=0)
    ...
    X     = ((np.asarray((periods,A1,A2,A3,phi21,phi31))-mean)/scale).reshape(1,-1)
