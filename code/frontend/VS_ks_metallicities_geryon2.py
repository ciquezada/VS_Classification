...
PROGRAM_TITLE += '-'*40+"\n"
PROGRAM_TITLE += "\n"
PROGRAM_TITLE += ' VS extractor de metalicidades (Geryon2 mode)'+"\n"
PROGRAM_TITLE += "\n"
...
# ks_metallicities
cd \"{ks_metallicities}\"
PROGRAM=\"adapter_Ks_metallicity.py\"
        """
...
def validate_params(params):
    ...
    params[1]["1.- Extraer metalicidades"] = elec_in[params[1]["1.- Extraer metalicidades"]]
    params[1]["2.- Ejecutar pymerlin"] = elec_in[params[1]["2.- Ejecutar pymerlin"]]
    return params
    ...
    while True:
        ...
        if i >= n:
            ...
            if final_election == 'y' or final_election == '':
                ...
                break
        election = "ELECTION"
        election_msg = ["Parametros Iniciales", "Extractor de Metalicidades"]
...
def write_join_features_python(params, subdir_list):
    ...
    new_python_script = f"{output_dir}{os.sep}join_metallicities.py"
...
"""
    ...
    if params[1]["1.- Extraer metalicidades"]:
        python_str += """
metallicities_df = pd.concat([pd.read_csv(f\"{dir}{os.sep}metallicities.csv\", sep=\" \")
                                    for dir in subdir_list], ignore_index=True)
"""
        python_str += f"""
metallicities_df.to_csv(\"{output_dir}{os.sep}metallicities.csv\", sep=\" \", index=False)
del(metallicities_df)
"""
    python_str += f"""
...
try:
    os.remove("join_metallicities.py")
except:
    print("Error while deleting file : ", "join_metallicities.py")
...
def run_on_geryon_ks_metallicities(params):
    subdir_list = []
    if not int(params[1]["1.- Extraer metalicidades"]):
        return subdir_list
    execution_title = params[0]["1.- Nombre del sample (Carpeta con outputs)(sin espacios)"]
    output_dir = params[0]["2.- Donde guardar la carpeta output"]
...
"""
    if int(params[1]["1.- Extraer metalicidades"]):
        write_join_features_python(params, subdir_list)
        RUN_script += "python join_metallicities.py\n"
...
def vs_ks_metallicites_geryon2_front():
    prev_params_path = f'{cwd}{os.sep}prev_ks_metallicities_params.txt'
    ...
    subdir_list = run_on_geryon_ks_metallicities(params)
    write_final_script(params, subdir_list)
    print("\nDone!")

if __name__=="__main__":
    vs_ks_metallicites_geryon2_front()
    if len(sys.argv)==1:
