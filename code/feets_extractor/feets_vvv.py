...
def load_features(info_file = "training_set.fnl", selected_features = []):
    ...
    data_path = cwd + os.sep  + ".." + os.sep + "datos_vvv" + os.sep + "o4_tset_all_vvv" + os.sep
...
def extractor_worker(arg):
    ...
    if not set(selected_features).isdisjoint(gp_dependent_features):
        params["FitGP"] = {"period": aov1, "gamma": 0.5}
    if not set(selected_features).isdisjoint(template_dependent_features):
        params["FitTemplate"] = {"period": min_aov} #min_aov para comparar las amplitudes
    if not set(selected_features).isdisjoint(fcomponents_dependent_features):
        params["FitFourier"] = {"period": aov1, "gamma": 0.5}
    ...
    return [row["vvv"], row["label"], aov1] + extr

if __name__=="__main__":
    selected_features = ['A1A2_ratio',
            'AC_std',
            'AD',
            	'Amplitude',
            	'AndersonDarling',
